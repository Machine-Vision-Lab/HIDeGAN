        ...
        if self.display_id > 0:  # show images in the browser using visdom
            ...
            if ncols > 0:        # show all the images in one visdom panel
                ...
                for label, image in visuals.items():
                    image_numpy = util.tensor2hsi(image)
    ...
    def print_current_losses(self, epoch, iters, losses, t_comp, t_data):
        ...
        if (self.loss_flag):
            for k,v in losses.items():
                header += ', %s' %(k)
            with open(self.log_csv, "a") as log_file:
                log_file.write('%s\n' % header)
            self.loss_flag = False
         
        message ='%d, %d, %.3f, %.3f' % (epoch, iters, t_comp, t_data)
        for k,v in losses.items():
            message += ', %.3f' %(v)
        with open(self.log_csv, "a") as log_file:
                log_file.write('%s\n' % message)
        ...
        message2print = '(epoch: %d, iters: %d, time: %.3f, data: %.3f) ' % (epoch, iters, t_comp, t_data)
        for k, v in losses.items():
            message2print += '%s: %.3f ' % (k, v)

        print(message2print)  # print the message
