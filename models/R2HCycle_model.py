...
class R2HCycleModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        Skeleton loss (optional): It has 2 components
            Single skeleton loss : lambda_skel_single * (||{Y(A) - Y(G_A(A))} + {Y(B) - Y(G_B(B))} ||*0.5)
            Cyclic skeleton loss : lambda_skel_cycle * (||{Y(A) - Y(G_B(G_A(A)) + {Y(B) - Y(G_B(G_A(B))||*0.5)
        ...
        if is_train:
            ...
            parser.add_argument('--lamba_skel_cycle', type=float, default='0.5', help='weight for canny edge loss for cycles')
            parser.add_argument('--lambda_skel_single', type=float, default='0.5', help='weight for canny edge loss for 1 GAN')
        return parser
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['D_A', 'G_A', 'cycle_A', 'D_B', 'G_B', 'cycle_B','idt_A','idt_B','skel_single','skel_cycle']
    ...
    def backward_G(self):
        ...
        lamba_skel_cycle = self.opt.lamba_skel_cycle
        lambda_skel_single = self.opt.lambda_skel_single 
        ...
        if lambda_skel_single > 0:
            self.loss_skel_single = self.skeleton_loss()[0] * lambda_skel_single
        else:    
            self.loss_edge_single = 0
        if lamba_skel_cycle > 0:
            self.loss_skel_cycle = self.skeleton_loss()[1] * lamba_skel_cycle
        else:    
            self.loss_edge_cycle = 0
            
        # combined loss and calculate gradients
        self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B  + self.loss_idt_A + self.loss_idt_B + self.loss_skel_single + self.loss_skel_cycle
    ...
    def hsi2rgb(self, hsi_batch,channels = 'first'):
        ...
        (HINT for the magic numbers : they are found using the wavelengths of 31 channels and R G B.) 
        
        Arguments -
        'hsi_batch' : batch of HSI image
        'channels' : indicate if the image has channels as first dimension or last dimension.
        """
        R_ = 28
        G_ = 16
        B_ = 8
    ...
    def skeleton_loss(self):
        ...
        for i in range(batch_size):
            ...
            #converting to gray scale and denormalizing
        ...
        return loss_skel_single, loss_skel_cycle
